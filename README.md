...
Instead of a pill behind the current item, the bar's liquid arrives around whatever the pointer is over. Nothing slides sideways: a bead drops in from above, another rises from below, a pool gathers a beat behind them, and a loose glob at each end is drawn in and pinches off. The five shapes are merged by a gaussian blur and an alpha threshold, the same identity that turns a sum of kernels into a metaball field, so the meeting is a merge rather than an overlap. The beads come to rest inside the pool's own box, so once they have landed the silhouette is a pill again and the arrival is all they were for. The pool is the page's own liquid welling through the bar, lit the way the mass that swallows a card afloat is lit rather than painted flat, and the label over it takes that liquid's ink.
