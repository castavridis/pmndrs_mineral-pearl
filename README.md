...
The bar wears the callout's iridescence over its own liquid. It is registered with the nacre stage as a `sheer` card: the shader runs as it does for a callout, but the body it would draw is dropped and only what the surface catches — the nacre's spectrum, the glitter, the thin film — is composited, its own brightness standing as its alpha. So the pearl the bar's `Surface` draws is still the pearl, with the cards' shine lying on it. The card also asks for `sheen` and `film` above a callout's, since a long shallow slab has little fresnel to carry either, and turns the ambient `blobs` off, which on a bar read as blotches rather than as liquid under glass. The pointer's droplet rests on the mark.
