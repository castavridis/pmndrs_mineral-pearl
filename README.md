...
The swallow is tunable. `swallow` takes any of the numbers in `ANNOUNCEMENT_SWALLOW` (`announcement/swallow.ts`) — droplet size, density, heap height, settle, whether the mass is lit, the rest and press depths, the bleed and its own viscosity — and `/dev/announcement` drives the same set from a panel, so a look can be found there and pasted back into the defaults. Dismiss a banner, tune, hit restore, dismiss again.
