...
Instead of a pill behind the current item, the bar's liquid is drawn in from its top and bottom edges and pools around whatever the pointer is over, clinging to it as it moves. Three shapes make it — a tongue down from the top, one up from the bottom, and the pool itself — merged by a gaussian blur and an alpha threshold, the same identity that turns a sum of kernels into a metaball field. The tongues are narrower than the pool so the blur necks them where they meet, which is what reads as liquid drawn up rather than a box. The pool is the page's own liquid welling through the bar, and the label over it takes that liquid's ink.
