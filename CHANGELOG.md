...
| The pool arrives rather than slides, and it arrives as beads | Asked for: globs in from the top and the bottom, pooling together. A tongue reaching to the bar's edge cannot work here — the row is the bar's full height, so there is no bar left above or below to reach across, and the two slivers that were left pinched the pill into a diamond under the threshold. Beads as deep as the pool drop in from outside the bar and come to rest inside the pool's box, so they add nothing to the resting silhouette. |
| Two loose globs at the pool's ends, absorbed | The same pull that draws a swallow's droplets back onto a slab. Each necks and pinches off as it goes under the threshold, so the pool grows fingers for a beat instead of arriving finished. |
| The pool is lit, not painted flat | It is the page's body welling through the bar, so it should read as the same liquid that swallows a card afloat. The shading sits inside the goo filter: the blur smears it across the merged silhouette, and the threshold reads alpha alone, which gradients never touch. |
