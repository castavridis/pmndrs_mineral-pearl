...
| The article launcher floats on the page's opposite body | Asked for: pearl on mineral and the other way about. It is the stand the nav bar already takes, and it separates the control from the page it sits on without giving it a border. Naming the liquid also opts the well out of the theme's masked switch, which is what keeps it the opposite rather than a patch of the page. |
| The announcement is submerged on load and surfaces when the page is ready | Asked for. It also covers the beat before the shaders have drawn: the banner spends it under the liquid, where nothing is expected to be legible, instead of sitting flat and then coming alive. The sink splashes only for a change of state, never for its opening one, so going under is free and the rise is the movement. `load` and the fonts say when, with a backstop in case either never resolves. |
