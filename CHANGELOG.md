...
| The liquid pools around the current item instead of a pill sliding behind it | A pill is a shape that happens to move; liquid drawn in from the bar's edges and clinging to the item is the bar behaving like what it is made of. Built from three shapes merged by a blur and an alpha threshold, so it necks and stretches on its own rather than being animated. |
