...
| The pill's accent is the page's body, not the mark's green | Asked for: mineral on dark, pearl on light. The green made the pill read as a state — a kind, the way a callout's accent is — when it is a window onto the liquid the page is already made of. It is the colour `--nav-pool` and the label's ink were keyed to all along. |
| Tried the callout's iridescence on the bar, and took it back | Asked for, then asked to be reverted. Registering the bar as an ordinary card painted a flat cream slab over the pearl its `Surface` draws; a `sheer` card that composited only the shine kept the pearl underneath but turned it silver. The bar wears the pond's own pearl again, which was what the request had meant by pearl all along. The experiment is in ca462c0 if it is ever wanted. |
| `spatter` on the splat, and the mark's blot turned down to 0.22 | Asked for a rounder, contained blot behind the logo. One number thins every group that flies, shortens what is left and flattens the lobes the launch speed carries, so the same sim gives a star at 1 and a round mass near 0. The blot is drawn larger to make up for the reach it loses. |
| The bento drops its tier headings, its tabs card, and puts the banner on top | Asked for. The elements are the point; naming the tiers only told you what to think of them. The banner sits above the bar, as it does on a page. |
| The bento is one centred column | Asked for, element by element. Rows of things side by side made the page a catalogue; a single column in the order a reader would meet them — the banner over the bar, then the things you use, then the plain controls — is closer to the page it stands for. Each cell is as wide as it wants up to its own cap, and the copy bar's hugs its actions: a cell wider than the bar leaves the sink's slab standing out to one side of it. |
| The bento carries the theme switch, dark and light only | Asked for, under the CTA's own toggle. `InkThemeToggle` takes the modes it cycles, so two of them make it a switch rather than a cycle, and the shell leaves its fixed one out for a page that carries its own (`AppShell toggle={false}`). A page about the two grounds has no use for following the system's choice. |
| ...and then the call to action *is* the switch | Asked for. A sample CTA that does nothing is a sample; one that changes the page's ground is the same slab doing real work, and the ink flooding out of the pressed face is the transition the page already had. The control under it disables the switch rather than a launcher, and the small toggle that briefly sat there is gone. |
| A halo behind a callout's headline and label | Asked for: the headline was hard to read. The liquid's crests come up close to the ink's own lightness where they pass under it. A text shadow in the page's ground — the ink's opposite in either scheme — holds the words without putting a plate behind them, and the headline gained a weight. |
| The banner's face wears a body, not a colour | Asked for: the relevant texture, pearl or mineral. The study's slab is a dry card, cream on the mineral and ink on the pearl. It now takes the *other* body's liquid — a slab wearing the one it floats in disappears into it, which is exactly what the first attempt did — so the contrast the study set is kept and the face gains that body's nacre, clouding and iridescence. |
| The banner and the call to action fit what is in them | Asked for. The banner carried the glass banner's 652 × 94 box, so a line of text sat in a slab half again as large as it needed, and the padding asked for could not show. Both now hug their content: the banner up to the width it is given, the button with 1.5rem at its sides, and the cell each sits in hugs it back, since a sink draws its slab to the cell rather than to what is on it. |
| The room opens as the banner surfaces | Asked for. It held the banner's height from the first frame, so the page had stood aside for something that was not there yet. The slot now holds none while the banner is under and grows to the height it wants as the liquid gives it back, on the same curve the close uses. |
...
| ...and `pressHeave` says how much of it is not | Asked for, of the launcher: less impact on the button, a softer and more global press. The lean is right for the copy bar, where the hand lands on one action of several, and wrong for a single face standing as the primary call to action. At 0.9 the slab gives as a whole and the knock falls away with the lever it belonged to. |
| The switch sinks through the page's own liquid | Asked for: the sunken-spot settings. The page look already is the pair of saved presets — "sunken spot" on the dark ground, "sunken spot (nacre)" on the light — and the switch inherited all of it except the viscosity, which the control look was holding at 0.22 against their 0.4 and 1.3. It now leaves that to the page: a control that sinks to say it is unavailable should sink through the liquid the page is made of. Its radius stays 8 (the page's rule, not the preset's 16) and its bleed 44, since 96 would reach over the control beneath it. |
| A well never turns to mercury, this time for good | The pond was built with `mercuryOnSink` forced off for a well, and the effect that pushes prop changes into it handed the default straight back on the next render — so every well sank into quicksilver, which is what the disabled switch was showing. The rule now lives in both places. A well is a window onto the page's own surface, and the page does not turn to chrome because one slab in it went under. |
| Callout headlines are the page's ink on the light ground | Asked for. The kind's colour is chosen to carry against black mineral; on pale paper a pmndrs green has almost nothing to hold on to. The headline takes the page's ink there and the icon keeps the colour, so the kind is still said in the one place that can afford to say it. |
| The switch goes under faster when it is disabled | Asked for. `sinkSpeed` scales the spring and its damping together, so it is the same motion on a faster clock rather than a different one; the switch takes 2.4. Thick liquid is right for the look and slow for the news — a control that has just been taken away should be seen to go, not watched sinking. |
| The palette page edits the page look | Asked for. Every number of the look in force, live, in the same folders the sink panel uses, with a copy-back button that writes the `PageLook` literal for `look.ts` and a reset. The panel is rebuilt when the scheme changes or a preset is applied, rather than being written into, which would fight whoever is typing; and it writes the look keyed on its values' content, since leva hands back a fresh object each render and the look is what the page is drawn from. |
| ...and now it actually does, and saves and loads | Reported: edits did nothing. Three faults, in order. The panel read leva's `[values, set]` tuple as if it were the values, so every write was empty. The effect that pushes the stored look into the fields was keyed on leva's setter, which is a new function each render, so it put the stored look back over every edit the instant it was made. And the panel was built from the store's `look`, which lags a theme change by a render — enough, once the fields wrote back, to put the dark look into the light scheme's slot. The panel now reads the scheme the page is showing and writes only into it. A presets folder saves the look under a name and loads or deletes one, in the sink panel's flat shape, so a look saved here loads there and back. |
| "light pearl" is the light ground's default | Asked for; the preset was saved from the palette panel. A brighter cream than the study's, heavily clouded and deeply nacred, with coarse sparse glitter that catches hard. It also ships in `pond-presets.json` beside the two sunken-spot presets. The dark ground stays "sunken spot". |
| Dark by default, and the system's preference is not followed for now | Asked for. The page is designed on the black mineral first, and a first visit should see it. The store defaults to dark; a stored "system" from before is migrated to dark rather than left following the OS, while an explicit light or dark is kept; the shell's toggle offers only the two; and the CSS no longer turns the pre-JS paint light for a light-preferring OS. `resolveTheme` still understands "system", so following it again is a default and a toggle list away. |
| The bento is the front page, centred both ways, with no rules | Asked for. `/` serves it (the former home moved to `/dev/home`), and it keeps its own theme switch there, so the shell's corner toggle stays off. A page that short reads better held in the middle of the viewport than hung from the top, and a line across a liquid page reads as a seam in it; the column's own spacing says where one group ends. |
...
| The banner sits in block flow on the home page, not in the page's grid | The collapse could close its height but never the grid gap that came with it: a gap belongs to the parent's tracks, and a negative margin on the item cannot eat one — the track clamps at zero. Unmounting then dropped the page by 56 px in a frame. In block flow the spacing is the banner's own margin, which closes with the height; the row it lives in has no padding of its own, so an empty row is nothing. |
...
| The banner's face wears the ink splat's nacre | Asked for, in place of a blot at its corner (`bf5d486`, removed): the banner still surfaces and is swallowed, and its face now has the splat's surface tension at the rim, its iridescence where the surface curves away and the pointer's dent. The face is a hole in the pond's layer through which the DOM slab shows, so the nacre lives on the slab itself: an `InkSplat` in the slab's own colour, `flooded` to the slab's box from the first frame with a `meniscus` at the rim for the lip and highlight to sit in. It tips with the slab and goes under with it. Only where the pond runs; the fallback tiers keep the flat face. |
| On the light page the banner's face is the pond's mineral | Asked for. The dark page keeps the ink splat's nacre on cream; the light page's face is now the mineral body — dark, glittering, iridescent where the pointer dents it — rather than the splat's nacre on near-black. It is a `LiquidGround` of mineral inside the slab, at the page ground's and the callouts' 420 px unit so it is the same stone, reading the pointer from the window because the face's layer takes no pointer events. |
| A theme change brings a dismissed banner back | Asked for: the resurfacing that followed a theme change never reached a banner the visitor had already closed, because the page had unmounted it. The theme store now counts changes (`changes`, bumped when a change commits, with ink or without; not persisted), and `useDismissal()` gives a page `[gone, dismiss]` that remembers the count a dismissal happened at. Once a later change has run its course — its ink cleared — the banner is back: it mounts afresh under the new scheme's liquid, its room opens and it surfaces, as on load. A dismissal made while a change's ink is still landing belongs to that change, so it is not undone by it. The bento and `/dev/home` use it; `/dev/announcement` keeps its restore button. |
| `T` switches the theme, through the ink | Asked for. `ThemeHotkey`, mounted by the shell on every page, requests the other of dark and light with the splat landing where the pointer last was (the keyboard has no point of its own), or mid-viewport before it has moved. It stands aside while something is being typed into, with a modifier held so the browser's shortcuts keep working, on a held key's repeats, and while a change is under way. It works whether or not the bento's switch is disabled: it is the page's command, not the button's. The switches carry `aria-keyshortcuts="T"`. |
| A theme change takes the banner under and gives it back | Asked for: the banner resurfaces once the transition ends. It goes under 150 ms after the ink starts, stays under while the theme commits beneath the ink, and rises 300 ms after the reveal, into the new scheme's face — the way it came up on load. It is keyed on the theme store's `pending`, and the first wait means a change that skips the ink (reduced motion) never touches it. Its room stays open throughout: a change of colour does not move the page. |
| The pearl banner has no meniscus by default | Asked for. The dark page's nacre face runs to a hard edge unless `meniscus` is set; `/dev/announcement` opens with it off. |
| The theme change's ink is quicker and eases out | Asked for. The splat's flood waited 1.2 s for the blot and then spilled for 1.4 s on a cubic ease-in, a spill gathering pace; the whole change took 2.8 s. The transition now leaves the blot at 0.8 s and covers in 1.2 s on a cubic ease-out, away at once and settling into the corners, about 2.2 s in all, and the reveal's fade is 340 ms ease-out rather than 400 ms `ease`. `InkSplat` takes `floodStart`, `floodLen` and `floodEase` for this; the droplets swell on the same curve as the front. Every other splat keeps the study's timing. |
| A splat started at its end draws on a fresh page | The splat marked "no impact yet" with a negative start time, and a clock started at the end of a splat (reduced motion, and now `flooded`) is set back from now — negative for the first seconds of a page's life. Such a splat drew nothing. The marker is now its own value. |
| A splat measures its layout box, not its painted one | fiber measured the canvas with `getBoundingClientRect`, which includes transforms. Inside a slab scaled down under the liquid it took the scaled size and kept it, since a transform changing is not a resize. `offsetSize` measures the layout box. |
