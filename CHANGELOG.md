...
| The bar wears the callout's iridescence, `sheer` | Asked for. Registering the bar as an ordinary card painted a flat cream slab over the pearl its `Surface` already draws — the pond's pastel nacre went, which was the thing worth keeping. A sheer card drops the body and composites only what the surface catches, its brightness standing as its alpha, so the pearl shows through the shine. It also needed `sheen` and `film` above a callout's (a long shallow slab has almost no fresnel) and its ambient blobs off (on a bar they read as blotches). |
